...
class Trainer(TrainBase):
    ...
    def _init_model(self):
        ...
        # self.emb2cls = Emb2Class(cls_len=self.ClassLen,hash_bit=self.args.output_dim).to(self.rank)
        self.labelNet = LabelNet(code_len=self.args.output_dim, label_dim=self.ClassLen).to(self.rank)
        ...
        self.cls2emb = Class2Emb(self.ClassLen, self.args.output_dim)
        self.criterion = nn.CrossEntropyLoss()
        # self.proxy = Proxy_Anchor_adaptive_1_margin(nb_classes=self.ClassLen,sz_embed=self.args.output_dim)
        ...
        self.optimizer = BertAdam([
            ...
            {'params': self.labelNet.parameters(), 'lr': self.args.lr},
            {'params': self.cls2emb.parameters(), 'lr': self.args.lr},
#            {'params': self.emb2cls.parameters(),'lr':self.args.lr },
        ...
        #
        # with torch.no_grad():
        #     flops = torchprofile.profile_macs(model1, dummy_input)
    ...
    def train_epoch(self, epoch):
        ...
        for image, text, label, index in self.train_loader:
            ...
            prototype = self.cls2emb(torch.eye(self.ClassLen)).to(self.rank)

            img_loss, icls = self.suploss(hash_img, prototype, labels, )
            text_loss, tcls = self.suploss(hash_text, prototype, labels, )

            img_text_loss, _ = self.suploss(hash_img, prototype, labels, feat2=hash_text)
            text_img_loss, _ = self.suploss(hash_text, prototype, labels, feat2=hash_img)

            # ED >> >> >> Best
            # epoch, I - T: 67, mAP: 0.8387652635574341, T - I: 90, mAP: 0.819315731525421
            cls_loss = self.criterion(icls,labels) + self.criterion(tcls,labels)
            tot_hier = img_loss + text_loss + img_text_loss + text_img_loss + 0.1*cls_loss

            tot_hier.backward()

            self.optimizer.step()
            # times+=time.time()-start_time
            all_loss += img_loss + text_loss + img_text_loss + text_img_loss + 0.1*cls_loss
            t_lossl += text_loss.cpu().item()
            it_lossl += img_text_loss.cpu().item()
        ...
        # print(epoch)
        # print("\b\b\b\b\b\b loss:%.3f|Traintime:%.6f" % (all_loss, times))
        # f .write('Train | Epoch: %d | Loss: %.3f | Traintime:%.6f\n' % (epoch, all_loss, times))
